...
def read_svg_hack(svg_filename, dpi=300):
    ...
    image = pyvips.Image.new_from_file(svg_filename, dpi=dpi)
...
def imgdiff(image_file_a, image_file_b, output_file):
    ...
    base_image = (
        read_svg_hack(image_file_a)
        if image_file_a.endswith(".svg")
        else Image.open(image_file_a)
    )
    new_image = (
        read_svg_hack(image_file_b)
        if image_file_b.endswith(".svg")
        else Image.open(image_file_b)
    )
    ...
    if image_file_a.endswith(".svg"):
        base_image = ImageChops.add(
            base_image, ImageChops.invert(base_image.getchannel("A")).convert("RGBA")
        )
    if image_file_a.endswith(".svg"):
        new_image = ImageChops.add(
            new_image, ImageChops.invert(new_image.getchannel("A")).convert("RGBA")
        )
    ...
    if image_file_a.endswith(".svg") and False:
        ...
        dark_theme = (
            fsum(get_dominant_color_from_corner(base_image)) < 127 * 3
        )  # this is a fuck

    base_image_bw = (
        ImageChops.invert(ImageOps.grayscale(base_image))
        if dark_theme
        else ImageOps.grayscale(base_image)
    )
    new_image_bw = (
        ImageChops.invert(ImageOps.grayscale(new_image))
        if dark_theme
        else ImageOps.grayscale(new_image)
    )
    ...
    if "A" in base_image.mode or "A" in new_image.mode:
        ...
        alpha_blend = ImageChops.lighter(
            base_image.getchannel("A"), new_image.getchannel("A")
        )
    ...
    base_mask = ImageChops.multiply(
        ...
        Image.new(
            "RGB", background.size, color=(200, 0, 200) if dark_theme else (0, 200, 200)
        ),
    )  # )
    if DEBUG:
    ...
    new_mask = ImageChops.multiply(
        ...
        Image.new(
            "RGB", background.size, color=(0, 200, 200) if dark_theme else (200, 0, 200)
        ),
    )  # )
    if DEBUG:
    ...
    output_image = ImageChops.subtract(
        ImageChops.subtract(
            Image.new("RGB", background.size, color=(255, 255, 255)), base_mask
        ),
        new_mask,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "base_image", type=str, nargs=1, help="Base image for comparison"
    )
    parser.add_argument(
        "new_image",
        type=str,
    ...
    parser.add_argument(
        "-o", type=str, nargs=1, help="Output file (stdout if not specified)"
    )
